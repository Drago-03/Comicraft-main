...
const ContactInfo = ({ icon: Icon, title, content, link }: any) => (
  <motion.div whileHover={{ scale: 1.02 }} className="flex items-start space-x-4 p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-red-500/30 hover:bg-white/10 transition-all group">
    <div className="p-3 bg-black/40 rounded-xl border border-white/5 group-hover:border-red-500/30 group-hover:bg-red-500/10 transition-colors">
      <Icon className="w-6 h-6 text-white/70 group-hover:text-red-400" />
    </div>
    <div>
      <h3 className="font-semibold text-white/50 text-sm tracking-wider uppercase mb-1">{title}</h3>
      {link ? (
        <Link href={link} className="text-base font-medium text-white hover:text-red-300 transition-colors">
          {content}
        </Link>
      ) : (
...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-red-500/30 font-sans relative overflow-hidden">
      {/* Background Decor */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-0 right-0 w-[50vw] h-[50vw] bg-red-500/10 blur-[150px] rounded-full translate-x-1/3 -translate-y-1/3" />
      ...
      <div className="container mx-auto px-6 py-24 relative z-10 max-w-7xl">
        ...
        <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="text-center mb-20 max-w-3xl mx-auto">
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs font-semibold tracking-wider text-red-400 mb-6">
          ...
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl font-bold tracking-tighter mb-6 relative">
            Establish <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-600">Contact</span>
          </motion.h1>
              ...
              <AnimatePresence mode="wait">
                {isSubmitted ? (
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="text-center py-16 flex flex-col items-center">
                    <div className="w-20 h-20 rounded-full bg-red-500/20 flex items-center justify-center border border-red-500/30 mb-8 relative">
                      <div className="absolute inset-0 rounded-full border border-red-500/30 animate-[ping_2s_ease-out_infinite]" />
                  ...
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="mb-8">
                       <h2 className="text-2xl font-semibold mb-2 flex items-center gap-2"><Hexagon className="w-5 h-5 text-red-400" /> Secure Terminal</h2>
                       <p className="text-white/40 text-sm">All entries are encrypted and transmitted directly to the Comicraft support mainframe.</p>
                    </div>
                      ...
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <FormField control={form.control} name="name" render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input placeholder="John Doe" {...field} className="bg-black/50 border-white/10 focus-visible:ring-red-500 h-14 rounded-xl text-white placeholder:text-white/20 transition-all hover:bg-black/70" />
                              </FormControl>
                          ...
                          <FormField control={form.control} name="email" render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input placeholder="john@domain.com" {...field} className="bg-black/50 border-white/10 focus-visible:ring-red-500 h-14 rounded-xl text-white placeholder:text-white/20 transition-all hover:bg-black/70" />
                              </FormControl>
                              <FormMessage className="text-rose-400" />
                            </FormItem>
                          )} />
                        ...
                        <FormField control={form.control} name="subject" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="What do you need assistance with?" {...field} className="bg-black/50 border-white/10 focus-visible:ring-red-500 h-14 rounded-xl text-white placeholder:text-white/20 transition-all hover:bg-black/70" />
                            </FormControl>
                            <FormMessage className="text-rose-400" />
                          </FormItem>
                        )} />
                        <FormField control={form.control} name="message" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="Transmit your message here..." className="bg-black/50 border-white/10 focus-visible:ring-red-500 min-h-[160px] rounded-xl text-white placeholder:text-white/20 resize-none transition-all hover:bg-black/70 p-4" {...field} />
                            </FormControl>
                            <FormMessage className="text-rose-400" />
                          </FormItem>
                        )} />
                        <Button type="submit" disabled={isSubmitting} className="w-full h-14 bg-red-500 hover:bg-red-400 text-black font-bold text-lg rounded-xl transition-all shadow-[0_0_30px_rgba(16,185,129,0.2)] hover:shadow-[0_0_40px_rgba(16,185,129,0.4)] group overflow-hidden relative mt-8">
                          {isSubmitting ? (

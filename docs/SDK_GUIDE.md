# Comicraft SDK Developer Guide

## Overview

The Comicraft SDK provides developers with easy-to-use tools for integrating AI-powered storytelling
...
```bash
npm install @comicraft/sdk
# or
yarn add @comicraft/sdk
```

### Basic Usage

```typescript
import { ComicraftSDK } from '@comicraft/sdk';
...
```typescript
import { ComicraftError, APIError, ValidationError } from '@comicraft/sdk';
...
```typescript
import { useComicraft } from '@comicraft/react-sdk';

function StoryGenerator() {
  const { generateStory, loading, error } = useComicraft();
...
```typescript
import { ComicraftProvider } from '@comicraft/react-sdk';

function App() {
  return (
    <ComicraftProvider apiKey="your-api-key">
      <StoryGenerator />
    </ComicraftProvider>
  );
}
...
class StoryApp {
  private sdk: ComicraftSDK;

  constructor(apiKey: string) {
    this.sdk = new ComicraftSDK({ apiKey });
  }
...
- **Documentation**: [docs.comicraft.com](https://docs.comicraft.com)
- **GitHub**: [github.com/Drago-03/Comicraft](https://github.com/Drago-03/Comicraft)
- **Discord**: [Join our community](https://discord.gg/comicraft)
- **Email**: <support@comicraft.com>

...
# Installation Guide for Comicraft 🛠️
...
on your system. These are essential for running Comicraft smoothly:
...
Start by downloading the Comicraft source code to your local machine using Git. This step retrieves
...
1. **Open Your Terminal or Command Prompt**: Navigate to the directory where you want to store the
   Comicraft project.
2. **Clone the Repository**: Run the following command to clone the Comicraft repository:

   ```bash
   git clone https://github.com/Drago-03/Comicraft.git
   cd Comicraft
   ```
...
Comicraft relies on various libraries and packages to function. Installing these dependencies is
crucial for running the application.

1. **Ensure You're in the Project Directory**: If you haven't already navigated into the `Comicraft`
...
Comicraft uses environment variables to manage sensitive information (like API keys) and
...
With dependencies installed and environment variables set, you're ready to launch the Comicraft
application locally.
...
2. **Access Comicraft in Your Browser**: Open your preferred web browser and navigate to:

   ```
   http://localhost:3000
   ```

   You should see the Comicraft application interface load, confirming a successful setup.
...
2. **Configure Wallet for Monad Testnet**: Set your wallet to the Monad Testnet network for
   ...
   - **Currency Symbol**: MONAD Add this network manually in your wallet settings if Comicraft
     doesn't prompt an automatic switch.
3. **Connect Wallet in Comicraft**: In the Comicraft application (at `http://localhost:3000`), click
...
4. **Obtain Test Tokens (If Needed)**: For testing on Monad Testnet, you may need test tokens for
   ...
   or check [GitHub Discussions](https://github.com/Drago-03/Comicraft/discussions) for updates.
...
[GitHub Discussions](https://github.com/Drago-03/Comicraft/discussions) for support.
...
[Back to Top](#installation-guide-for-comicraft-️)

...
# Smart Contracts on Comicraft

<div align="center">
  <img src="../../public/Comicraft.png" alt="Comicraft Logo" width="300" />
</div>

Comicraft leverages blockchain technology through smart contracts to enable the creation, ownership,
and trading of unique digital stories as Non-Fungible Tokens (NFTs) on the Monad blockchain. This
guide provides an overview of the smart contracts used in Comicraft, their functionality, and how
...
Comicraft. As the platform evolves, contract addresses, ABIs, and functionalities may be updated.
...
- [Comicraft Smart Contract Overview](#comicraft-smart-contract-overview)
...
terms of an agreement when predefined conditions are met. In the context of Comicraft, smart
...
Comicraft uses the Monad blockchain (currently Testnet, with Mainnet planned for the future) for its
smart contract operations, benefiting from its scalability and compatibility with Ethereum Virtual
Machine (EVM) standards.

## Comicraft Smart Contract Overview

The primary smart contracts in Comicraft are designed to support the following functionalities:
...
The core smart contract in Comicraft is the NFT Minting Contract, responsible for creating story
NFTs:
...
Developers and advanced users can interact with Comicraft smart contracts in several ways:

1. **Through Comicraft Frontend**:
   - The easiest way to interact with the contracts is via the Comicraft web interface. When you
...
4. **Remix or Truffle**:
   - For development or testing, use tools like Remix IDE or Truffle Suite to deploy, test, or
     interact with Comicraft smart contracts on the Monad Testnet.
...
As Comicraft is in active development, specific contract addresses for the Monad Testnet are not yet
...
1. Check the Comicraft GitHub repository under a `contracts` or `deployments` directory (if
   available).
...
input/output formats. The ABI for Comicraft NFT contracts is not yet publicly available as contracts
...
- **Contract Verification**: Ensure you're interacting with the official Comicraft contract
...
- **Audits**: Comicraft smart contracts will undergo security audits as they near production
...
[GitHub Discussions](https://github.com/Drago-03/Comicraft/discussions) or refer to the
[FAQ](../FAQ.md).
...
Understanding and interacting with Comicraft smart contracts opens up opportunities to build on top

...
describe('Bug Condition Exploration: Server Startup and CORS', () => {
  ...
  describe('Property 1: CORS Configuration - sdk-server.js Multi-Origin Support', () => {
    ...
    test('should allow cross-origin requests from comicraft.vercel.app', async () => {
      // **EXPECTED OUTCOME ON UNFIXED CODE**: This will FAIL
      // The current single-origin CORS config rejects this origin
      
      const response = await request(app)
        .get('/sdk/health')
        .set('Origin', 'https://comicraft.vercel.app');
      
      // Expected behavior: CORS headers should be present
      expect(response.headers['access-control-allow-origin']).toBe('https://comicraft.vercel.app');
      expect(response.headers['access-control-allow-credentials']).toBe('true');
      expect(response.status).toBe(200);
    });
    ...
    test('should allow cross-origin requests from comicraft.xyz', async () => {
      // **EXPECTED OUTCOME ON UNFIXED CODE**: This might PASS
      // This is the default origin in the current config
      
      const response = await request(app)
        .get('/sdk/health')
        .set('Origin', 'https://comicraft.xyz');
      
      // Expected behavior: CORS headers should be present
      expect(response.headers['access-control-allow-origin']).toBe('https://comicraft.xyz');
      expect(response.headers['access-control-allow-credentials']).toBe('true');
      expect(response.status).toBe(200);
    });
  });

...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ── Generate real panel data using Gemini 2.5 Flash Image ────
      const { generateImage } = require('../services/geminiService');
      const beatLabels = ['Intro', 'Conflict', 'Climax', 'Resolution'];
      const panels = [];

      // Generate in small batches to avoid timeouts while still parallelizing
      const batchSize = 3;
      for (let i = 0; i < Math.min(totalPanels, 24); i += batchSize) {
        const batchPromises = [];
        
        for (let j = 0; j < batchSize && i + j < Math.min(totalPanels, 24); j++) {
          const index = i + j;
          const pageNum = Math.floor(index / panelsPerPage) + 1;
          const panelNum = (index % panelsPerPage) + 1;
          const beatIndex = Math.min(Math.floor(index / (totalPanels / 4)), 3);
          const beat = parsedBeats[beatIndex] || beatLabels[beatIndex];
          const beatDescription = typeof beat === 'string' ? beat : beat.description || '';
          
          const cameraDir = ['wide shot', 'medium shot', 'close-up', 'action shot', 'reaction shot'][index % 5];
          const style = stylePreset || 'manga';
          
          const prompt = `A ${style} style comic panel. Characters: ${heroName || 'Hero'}. Action/Scene: ${beatDescription}. Camera: ${cameraDir}. Intricate, highly detailed drawn comic book panel.`;
          ...
          batchPromises.push(
            (async () => {
              ...
              return {
                ...
                imageUrl: imageUrl,
                caption: `[${beatLabels[beatIndex]}] Panel ${panelNum} — ${beatDescription}`.trim(),
                dialogue: '',
                characters: [heroName || 'Hero', ...(index % 3 === 0 && parsedCoStarNames.length > 0 ? [parsedCoStarNames[0]] : [])],

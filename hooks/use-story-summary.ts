...
export function useStorySummary(): UseStorySummaryResult {
  ...
  const generateSummary = async (
    ...
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://comicraft-backend-api.onrender.com') + '/api/story-summaries', {
        method: 'POST',
        headers: {
  ...
  const fetchSummary = async (
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://comicraft-backend-api.onrender.com'}/api/story-summaries?storyId=${storyId}`, {
        method: 'GET',
      });
  ...
  const deleteSummary = async (id: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://comicraft-backend-api.onrender.com'}/api/story-summaries?id=${id}`, {
        method: 'DELETE',
      });

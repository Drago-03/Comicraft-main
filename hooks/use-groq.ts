...
export function useGroq(): UseGroqResult {
  ...
  const fetchModels = useCallback(async () => {
    try {
      ...
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://comicraft-backend-api.onrender.com') + '/api/groq/models');
      if (!response.ok) {
  ...
  const generate = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://comicraft-backend-api.onrender.com') + '/api/groq', {
          method: 'POST',
          headers: {

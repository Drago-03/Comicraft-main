...
- **Analytics Warehouse**: Configured Supabase S3/Iceberg analytics warehouse (`analytics-comicraft`) with engagement summary view (`analytics.story_engagement_summary`).
...
- **CORS missing origins** (`server/config/cors.js`): Added `https://comicraft.com`, `https://www.comicraft.com`, `https://comicraft.xyz`, `https://www.comicraft.xyz`, `https://comicraft-sdk-service.onrender.com`, and `http://localhost:3002` (SDK dev) to `allowedOrigins`. Previously these origins were blocked in production.

## [1.9.4] - 2026-03-10

### Fixed

- **Gemini "not_configured" in backend health check** (`render.yaml`): Added `GEMINI_API_KEY` (secret, `sync: false`) and `GEMINI_MODEL` (default `gemini-2.5-pro`) environment variable declarations to the `comicraft-backend-api` service in `render.yaml`. Without these entries the Render dashboard never exposes the fields, so `process.env.GEMINI_API_KEY` is always `undefined` at runtime — causing the `/api/health` endpoint to permanently report `gemini.status: "not_configured"`. After setting the actual key value in the Render dashboard, the Gemini service will report `status: "available"` and AI generation will use the Gemini chairman model as intended.
...
- **Backend API branding** (`server/backend.js`): Updated Swagger/OpenAPI `title`, `description`, `customSiteTitle`, landing page name, and startup log from `Comicraft` to `COMICRAFT`.
...
### Changed
- **Rebranding - Comicraft**: Executed platform-wide rebrand from "Comicraft" to "Comicraft" unifying the experience.
...
- **CORS**: Dynamic callback validation supporting 7+ origins (Vercel, Cloudflare Pages, localhost, Render, Comicraft domains)
...
- **wiki/API-Documentation.md**: Corrected production base URL to `comicraft-backend-api.onrender.com/api`.
...
- Implement core application structure, API routes, UI components, and Cloudflare Worker for Comicraft.
...
- **`server/backend.js`**: Swagger server URL changed from `process.env.URL || 'http://localhost:PORT'` → `process.env.PROD_URL || 'https://comicraft-backend-api.onrender.com/api'` with "Production" description. CORS origin default changed from `localhost:3000` → `https://comicraft.xyz`. Health check startup logs now use `PROD_URL`.
- **`server/sdk-server.js`**: CORS origin default changed from `localhost:3000` → `https://comicraft.xyz`.
...
- **`server/routes/helpbot.js`** [NEW]: `POST /api/helpbot/chat` endpoint integrating with Groq LLM API. Includes system prompt with Comicraft knowledge base and fallback message when `GROQ_API_KEY` is not configured.
...
- **`Comicraft/`**: Removed duplicate nested project scaffold (had its own `package.json`, `next.config.js`, etc.) — never imported by root project.
...
- **Updated `.gitignore`**: Added `Comicraft/` and `path/` to prevent re-creation of accidental nested dirs. Removed duplicate `.DS_Store` entry.

#### Files Deleted
`Comicraft/`, `path/`, `deployment/`, `temp.md`, `.zap/`, `.npm-cache/`, `.DS_Store`, `app/test-buttons/`, `components/nft-purchase.tsx`, `components/ui/tooltip.tsx.backup`, `Dockerfile`, `docker-compose.yml`, `.env.docker`, `.dockerignore`, `tailwind.config.ts`
...
- **`app/layout.tsx`**: `VERCEL_URL` environment variable fallbacks replaced with `CF_PAGES_URL` (Cloudflare Pages runtime variable); default image/splash URLs point to `comicraft.xyz`.
...
- **`MIGRATION-TO-CLOUDFLARE.md`**: Migration guide documenting how to deploy to Cloudflare Pages, configure env vars, set up the custom domain `comicraft.xyz`, and troubleshoot common issues.
...
- `NEXT_PUBLIC_URL` and related image URL defaults now fall back to `comicraft.xyz` instead of the old `comicraft.com` placeholder.
...
Go to **GitHub → IndieHub25/Comicraft → Settings → Secrets and variables → Actions**:

| Secret Name | Value |
|---|---|
| `CLOUDFLARE_API_TOKEN` | Cloudflare API token with Pages edit permission |
| `CLOUDFLARE_ACCOUNT_ID` | Your Cloudflare account ID (from Cloudflare dashboard) |
| `NEXT_PUBLIC_API_URL` | `https://comicraft-backend-api.onrender.com` |
...
- **README Modernization**: Replaced top deployment badges with a clean Comicraft logo-centered layout, added "Built by Indie Hub" credit, and integrated a GitHub stargazers request to improve premium branding.
- **Wiki Standardization**: Injected the Comicraft logo header systematically across all Markdown files in the `wiki/` directory for visual consistency.
...
- **Footer Brand**: Replaced logo image/container with bold italic "Comicraft" branded text.
...
#### Added
- **Neon "COMICRAFT" Footer Branding**: Large Bangers-font branded heading at the bottom of the footer with a custom `neon-flicker` CSS animation that simulates a faulty neon sign — random blinks, flickers, and steady glow intervals
...
- **Header Wordmark**: "COMICRAFT" text in Bangers font displayed next to the logo in the header
...
- **Full-width Neon Sign**: COMICRAFT neon branding now spans the entire screen width on all devices (phone, tablet, laptop, TV) using `w-screen -ml-[50vw]` breakout technique
...
- **`components/footer.tsx`**: Added neon "COMICRAFT" branding section at the bottom of the footer
...
- **Why Comicraft Section**: Feature showcase with Lightning-Fast AI, True Ownership, and Vibrant Community cards
...
- Core Comicraft platform functionality

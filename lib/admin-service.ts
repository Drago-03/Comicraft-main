...
export function getAdminAvatarUrl(): string {
  ...
  return 'https://api.dicebear.com/7.x/bottts/svg?seed=Comicraft&backgroundColor=6d28d9';
}
...
export function useAdminInteractions() {
  ...
  const likeStory = async (storyId: string) => {
    ...
    if (success) {
      toast({
        ...
        description: 'You liked this story as Comicraft admin',
      });
    } else {
      toast({
        variant: 'destructive',
  ...
  const dislikeStory = async (storyId: string) => {
    ...
    if (success) {
      toast({
        title: 'Action successful',
        description: 'You disliked this story as Comicraft admin',
      });
    } else {
      toast({
        variant: 'destructive',
